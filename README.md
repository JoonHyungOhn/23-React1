...
#### 15.1.1 CSS  
* 웹 개발을 할 때 스타일링을 하기 이해 가장 대표적으로 사용되는 것이 <b>CSS</b>이다.  
* CSS는 Cascading Style Sheets의 약자로 스타일링을 위한 일종의 언어라고 생각하면 된다.  
* <b>Cascading</b>이라는 단어는 <b>계단식</b>이라는 뜻을 가지고 있다.  

#### 15.1.2 CSS 문법과 선택자  
* CSS는 크게 선택자(selector)와 스타일로 구성되어 있다.  
* 아래 코드를 보면 <b>선택자를 먼저 쓰고 이후에 적용할 스타일을 중괄호 안에 세미콜론(;)으로 구분하여 하나씩 기술</b>한다.  
```css
...
* 이때 컨테이너에 포함되는 엘리먼트들이 바로 플렉스박스의 아이템(flex item)이 된다.
...
* 글꼴과 관련된 CSS 속성으로는 font-size가 있다.  
* font-size의 값으로는 px, em, rem, vw 등의 단위를 사용할 수 있다.    
* px은 고정된 값, em은 사용자가 브라우저 글꼴의 크기를 변경하게 해주는 것으로 em은 상대 글꼴, px은 절대 글꼴이라고 생각하면 된다.  
...
* RGB 컬러값: rgb(255, 0, 0)  
* RGBA 컬러값: rgba (255, 0, 0, 0.5) (뒤의 55는 알파값이다)   
...
#### * 플렉스박스  
    ...
    ■ flex-direction: 아이템들이 어떤 방향으로 배치할 것인지를 지정하기 위한 속성  
     ...
     * cross-axis를 기준으로 함  
    ■ justify-content  
     * 컨테이너 내에서 아이템들을 어떻게 나란히 정렬할 것인지를 결정하기 위한 속성  
     * main axis을 기준으로 함  
